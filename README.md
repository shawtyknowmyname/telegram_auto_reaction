...
1. Перейдите на сайт [my.telegram.org](https://my.telegram.org) и войдите с вашим Telegram-аккаунтом, указав номер телефона и код подтверждения.
...
3. Заполните форму создания приложения:
   ...
   - **URL** — можно указать любой сайт (например: `https://example.com`)
   - **Platform** — выберите `Desktop`
   - **Description** — можно оставить пустым или указать краткое описание
4. Нажмите **Create application**.
5. Скопируйте выданные **API ID** и **API HASH** — они понадобятся при первом запуске приложения.
